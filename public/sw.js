...
workbox.precaching.precacheAndRoute([
  {url: '/member/offline', revision: '1' },
  {url: '/mobilekit/assets/img/icon/offline.png', revision: '1' },
]);
...
workbox.routing.registerRoute(
  ({url}) => url.pathname.startsWith('/member/dashboard'), // Butuh cek login, slider dan sidebar dinamis SSR
  new workbox.strategies.NetworkFirst()
);
workbox.routing.registerRoute(
  ({url}) => url.pathname.startsWith('/member/feeds'), // Butuh cek login
  new workbox.strategies.NetworkFirst()
);
workbox.routing.registerRoute(
  ({url}) => url.pathname.startsWith('/member/feed'), // Butuh cek login
  new workbox.strategies.NetworkFirst()
);
workbox.routing.registerRoute(
  ({url}) => url.pathname.startsWith('/member/videos'), // Butuh cek login
  new workbox.strategies.NetworkFirst()
);
workbox.routing.registerRoute(
  ({url}) => url.pathname.startsWith('/member/video'), // Butuh cek login
  new workbox.strategies.NetworkFirst()
);
workbox.routing.registerRoute(
  ({url}) => url.pathname.startsWith('/member/santri'), // Butuh cek login
  new workbox.strategies.NetworkFirst()
);
